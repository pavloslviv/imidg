 ...
 */
(function (sr) {
    sr.classes.views.OrderDetails = sr.classes.views.Base.extend({
        ...
        template: sr.core.loadTemplate('tmpl_shop_order_details'),
        templateItems: sr.core.loadTemplate('tmpl_shop_order_details_items'),
        templateShipment: sr.core.loadTemplate('tmpl_shipment_form'),
        templatePayment: sr.core.loadTemplate('tmpl_payment_form'),

        initialize: function(data){
            var  that = this;
            _.bindAll(this,'render','renderItems','renderShipment');
            if(sr.data.orders){
                this.model = sr.data.orders.get(data.orderId);
            }
            if(!this.model){
                this.model = new sr.classes.models.Order({id: data.orderId});
            }
        ...
        renderItems: function(){
            this.$('#order-items-container').html(this.templateItems({
        ...
        renderShipment: function(){
            var data = this.model.get('shipment_data');
            var method = this.model.get('shipment');
            if (!data) {
                data = {};
            }
            if (!method) {
        ...
        onStatusChange: function(e){
            ...
            if(!confirm('Сменить статус заказа?')) {
