...
class ShopOrders extends Component
{
    ...
    public function listItems()
    {
        ...
        $orderList = new DBCollection('sr_shop_order', array('id', 'customer_id', 'customer_name', 'customer_phone','date', 'status', 'total'));
        ...
        $customerIds = array_unique(Helpers::array_pluck($orderList->data,'customer_id'));
        $customerList = new DBCollection('sr_customer', array('id', 'mail','name','phone'));
        $customerList->fetch('`id` in ('.implode(',',$customerIds).')');
        foreach($orderList->data as &$order){
            $order['customer']=$customerList->data[$order['customer_id']];
    ...
    public function  addItem(){
        ...
        if($order->get('status')!=='new' && $order->get('status')!=='processing' && $order->get('status')!=='oneClick'){
